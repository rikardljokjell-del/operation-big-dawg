(()=>{
  ...
  function rollLoot(playerState,lvl){const owned=uniq(playerState.owned_pokemon),complete=owned.length>=151,max=lvl>=8?151:148,pool=complete?Array.from({length:151},(_,i)=>i+1):Array.from({length:max},(_,i)=>i+1).filter(id=>!owned.includes(id));return pool.length?sample(pool,Math.min(lootCount(),pool.length)):[]}
  ...
  async function ensureEncounter(){
    ...
    if(p.current_cycle<1){p={...p,current_cycle:1,current_leader:0,damage:0,leader_defeated:false,current_loot:rollLoot(p,lvl),seen_leaders:[...new Set([...p.seen_leaders,0])]};changed=true}
    else if(!p.current_loot.length&&p.owned_pokemon.length<151){p={...p,current_loot:rollLoot(p,lvl)};changed=true}
  ...
  function nextEncounterState(p,lvl){
    ...
    const base={...p,current_cycle:Number(p.current_cycle||0)+1,current_leader:idx,initial_round_complete:complete,damage:0,leader_defeated:false,current_loot:[]};
    return {...base,current_loot:rollLoot(base,lvl),seen_leaders:[...new Set([...base.seen_leaders,idx])]}
  }
  ...
  function renderGym(){
    if(!cache)return;const b=ensureUi(),p=cache,l=leader(),hp=Math.max(0,HP-p.damage),pct=Math.max(0,Math.min(100,hp/HP*100)),loot=uniq(p.current_loot),owned=uniq(p.owned_pokemon),party=uniq(p.active_party),complete=owned.length>=151,team=teamHelp(),lvl=levelFor(activeName());
    b.hidden=false;b.innerHTML=`<div class="boss-topline"><span class="boss-alert">GYM LEADER APPEARING!</span><span class="boss-clock">PERSONAL STORY</span></div>
      ...
      <div class="carried-head"><span>${complete?'CARRIED POKÉMON':'STEAL IF YOU WIN'}</span><b>${complete?'GYMDEX COMPLETE':loot.length?`${loot.length} NEW`:(lvl<8?'#149–151 LOCKED':'NO NEW LOOT')}</b></div>
      ...
      <div class="party-strip"><span>PARTY ${party.length}/6 · +${party.length*2}% DMG · TEAM +${Math.round(team.bonus*100)}%</span><div>${party.map(id=>pokeArt(id,true)).join('')}${Array.from({length:6-party.length},()=>'<i>+</i>').join('')}</div></div>`;
  ...
  function damageFor(m,d){const l=leader(),party=uniq(cache.active_party).length,team=teamHelp();let mult=1,notes=[];if(m.type===l.weak){mult*=1.35;notes.push('SUPER EFFECTIVE')}if(d.type==='strength'&&m.type==='power'){mult*=1.12;notes.push('STRENGTH BOOST')}if(d.type==='cardio'&&m.type==='engine'){mult*=1.12;notes.push('CARDIO BOOST')}mult*=1+Math.min(.18,Math.max(0,Number(d.levelAfter||levelFor(activeName()))-3)*.03);if(party){mult*=1+party*.02;notes.push(`PARTY +${party*2}%`)}if(team.bonus){mult*=1+team.bonus;notes.push(`TEAM HELP +${Math.round(team.bonus*100)}%`)}const crit=Math.random()<Math.min(.55,(m.crit||.10)+(party>=6?.05:0));if(crit){mult*=1.45;notes.push('CRITICAL')}mult*=.96+Math.random()*.08;return{damage:Math.max(5,Math.round(m.power*mult/5)*5),notes}}
  ...
  const onWorkout=d=>{scheduleRefresh(0);const lvl=levelFor(activeName());if(lvl<3)return;const levelUp=Number(d?.levelAfter||lvl)>Number(d?.levelBefore||lvl);const delay=TEST_MODE?(levelUp?1200:350):(levelUp?7800:800);setTimeout(async()=>{await refreshGym();if(cache&&!cache.leader_defeated)attackReady({...d,person:d?.person||activeName(),type:d?.type||'strength',levelAfter:lvl})},delay)};
