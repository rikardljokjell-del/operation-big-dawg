(()=>{
  if(!window.__OBD_PREVIEW__||window.__obdWildManualCatchPreview)return;
  window.__obdWildManualCatchPreview=true;
  ...
  const PIN='1337';
  const STORAGE='obd_preview_wild_catch_pending_v1';
  let pending=null;
  let busy=false;
  let observer=null;
  ...
  function savePending(value){
    pending=value||null;
    const map=readMap();
    const id=String(value?.player_id||activeId()||'');
    if(id){if(value)map[id]=value;else delete map[id];writeMap(map)}
    renderAction();
  }
  function clearPending(id=activeId()){
    const map=readMap();
    if(id)delete map[id];
    writeMap(map);
    if(!pending||String(pending.player_id)===String(id))pending=null;
    renderAction();
  }

  function ensureStyles(){
    if(document.getElementById('wildManualCatchPreviewStyle'))return;
    const style=document.createElement('style');
    style.id='wildManualCatchPreviewStyle';
    style.textContent=`
  ...
  function ensureReadyOverlay(){
    ensureStyles();
    let overlay=document.getElementById('wildCatchReadyOverlay');
    if(overlay)return overlay;
    overlay=document.createElement('div');
    overlay.id='wildCatchReadyOverlay';
    overlay.className='wmc-ready-overlay';
    overlay.hidden=true;
    ...
    overlay.querySelector('[data-wmc-ready-ok]')?.addEventListener('click',()=>{
      overlay.hidden=true;
  ...
  function scrollToWild(){
    renderAction();
    const block=document.getElementById('wildPokemonBlock');
    if(!block)return;
    block.scrollIntoView?.({behavior:'smooth',block:'center'});
    block.classList.remove('wmc-pulse');
    requestAnimationFrame(()=>block.classList.add('wmc-pulse'));
    setTimeout(()=>block.classList.remove('wmc-pulse'),850);
  }

  function actionHtml(){
    return `<div class="wmc-action" data-wmc-action><small>This workout unlocked a catch attempt. Start the encounter when you're ready.</small><button type="button" class="wmc-catch-btn" data-wmc-catch><span class="wmc-ball" aria-hidden="true"></span><span class="wmc-catch-copy"><b>THROW POKÉ BALL TO CATCH IT!</b><span>Tap to start the catch battle</span></span></button><div class="wmc-error" data-wmc-error></div></div>`;
  }
  ...
  function renderAction(){
    ...
    const block=document.getElementById('wildPokemonBlock');
    if(!block)return;
    const existing=block.querySelector('[data-wmc-action]');
    const valid=pending&&String(pending.player_id)===activeId()&&block.querySelector('.wild-main')&&!block.querySelector('.wild-cooldown');
    if(!valid){existing?.remove();return}
    if(existing)return;
    block.insertAdjacentHTML('beforeend',actionHtml());
  }

  async function validateStored(candidate){
    if(!candidate?.player_id||!candidate?.workout_id)return false;
    try{
      const response=await fetch(PREVIEW_WILD,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'wild_validate',player_id:candidate.player_id,workout_id:candidate.workout_id,level:candidate.level||activeLevel(),pin:PIN})});
      const data=await response.json();
      if(!response.ok||!data?.eligible){clearPending(candidate.player_id);return false}
      pending={...candidate,level:candidate.level||activeLevel(),pokemon_id:data?.wild?.pokemon_id||candidate.pokemon_id||null};
      renderAction();
      return true;
  ...
  async function restorePending(){
    const id=activeId();
    if(!id){pending=null;renderAction();return}
    const candidate=readMap()[id];
    if(!candidate){pending=null;renderAction();return}
    pending=candidate;
    await validateStored(candidate);
  }

  async function startCatch(button){
    if(busy||!pending)return;
    const current={...pending};
    if(String(current.player_id)!==activeId())return clearPending(current.player_id);
    busy=true;
    button.disabled=true;
    const errorNode=document.querySelector('[data-wmc-error]');
    ...
    try{
      ...
      if(data?.battle_resolved||data?.wild?.status==='cooldown'||data?.already_resolved){clearPending(current.player_id)}
      else if(data?.eligible===false){clearPending(current.player_id)}
      window.dispatchEvent(new Event('pageshow'));
    }catch(error){
      console.warn('Manual Wild catch failed',error);
      const node=document.querySelector('[data-wmc-error]');
      if(node)node.textContent=String(error?.message||'Could not start battle. Try again.');
    }finally{
      busy=false;
      const live=document.querySelector('[data-wmc-catch]');
      if(live)live.disabled=false;
    }
  }

  window.addEventListener('obd-wild-catch-ready',event=>{
    const detail=event.detail||{},body=detail.body||{},validation=detail.validation||{};
    if(!body.player_id||!body.workout_id||!validation?.eligible)return;
    savePending({player_id:String(body.player_id),workout_id:String(body.workout_id),level:Number(body.level)||activeLevel(),pokemon_id:validation?.wild?.pokemon_id||null,created_at:new Date().toISOString()});
    showReadyNotice();
    setTimeout(renderAction,0);
  });

  document.addEventListener('click',event=>{
    const button=event.target?.closest?.('[data-wmc-catch]');
    if(button)startCatch(button);
  });
  ...
  window.addEventListener('obd-wild-status-refreshed',event=>{
    if(event.detail?.wild?.status==='cooldown')clearPending();
    else setTimeout(renderAction,0);
  });
  window.addEventListener('pageshow',()=>setTimeout(renderAction,300));

  const startObserver=()=>{
    if(observer||!document.body)return;
    observer=new MutationObserver(()=>renderAction());
    observer.observe(document.body,{childList:true,subtree:true});
  };
  ensureStyles();
  if(document.body)startObserver();else document.addEventListener('DOMContentLoaded',startObserver,{once:true});
  setTimeout(restorePending,500);
})();
