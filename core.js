...
const typesThisWeek=p=>{const w=currentWeek(),r=rowsForPlayer(p).filter(x=>mondayKey(x.created_at)===w);return{strength:r.filter(x=>x.workout_type==='strength').length,cardio:r.filter(x=>x.workout_type==='cardio').length,mobility:r.filter(x=>x.workout_type==='mobility').length}};
