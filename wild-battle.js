(()=>{
  if(window.__obdWildBattle)return;
  window.__obdWildBattle=true;
  ...
  function ensureStyles(){
    if(document.getElementById('wildBattleStyles'))return;
    const s=document.createElement('style');s.id='wildBattleStyles';s.textContent=`
  ...
  function ensureOverlay(){
    ensureStyles();let o=document.getElementById('wildBattleOverlay');if(o)return o;
    o=document.createElement('div');o.id='wildBattleOverlay';o.className='wbp-overlay';o.hidden=true;o.innerHTML='<div class="wbp-card" id="wildBattleCard" role="dialog" aria-modal="true"></div>';document.body.appendChild(o);return o;
  }
  function closeBattle(){const o=document.getElementById('wildBattleOverlay');if(o){o.hidden=true;o.style.display='none';o.setAttribute('aria-hidden','true');o.remove()}document.body?.classList.remove('obd-modal-open');document.documentElement?.classList.remove('obd-modal-open')}
  function openBattle(html){const o=ensureOverlay(),c=document.getElementById('wildBattleCard');c.innerHTML=html;o.style.display='grid';o.hidden=false;o.removeAttribute('aria-hidden');document.body.classList.add('obd-modal-open')}
  ...
  function battleHtml(state){
    ...
    return `<div class="wbp-top"><span class="wbp-kicker">WILD ENCOUNTER</span><span class="wbp-turn">TURN ${state.turns+1} · ${state.profile.tier}</span></div>
  ...
  async function attack(state,moveId){
    ...
    if(hit){state.wildHp=Math.max(0,state.wildHp-damage);document.getElementById('wildBattleCard')?.classList.add('wbp-impact');try{navigator.vibrate?.(crit?[35,25,55]:25)}catch{};await wait(190)}
  ...
  window.fetch=function(input,init){
    ...
    }catch(error){console.warn('Wild battle intercept failed',error)}
    return upstreamFetch(input,init);
  };
  window.fetch.__obdWildBattle=true;
