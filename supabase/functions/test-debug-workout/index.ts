...
const players=`${U}/rest/v1/players`,workouts=`${U}/rest/v1/workouts`;
...
async function getTestPlayer(id:string){if(!id)return null;const a=await api(`${players}?id=eq.${encodeURIComponent(id)}&select=id,name&limit=1`);const p=a?.[0];return p&&String(p.name||'').trim().toLocaleLowerCase('nb-NO')==='test'?p:null}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const b=await req.json(),id=String(b.player_id||''),p=await getTestPlayer(id);
    if(!p)return out({error:'Debug access only for player test'},403);
    if(b.action==='add'){
      const type=String(b.workout_type||'');if(!['strength','cardio'].includes(type))return out({error:'Bad request'},400);
      const created_at=await nextDate(id);
      const a=await api(workouts,{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({player_id:id,person:p.name,workout_type:type,created_at})});
      return out(a);
    }
    if(b.action==='undo'){
      const a=await rows(id);if(!a.length)return out({ok:true,deleted:false,reason:'NO_WORKOUT'});
      const target=a[0];await api(`${workouts}?id=eq.${encodeURIComponent(target.id)}`,{method:'DELETE'});return out({ok:true,deleted:true});
    }
